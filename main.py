...
def handle_print_vehicles(allowed_vehicles):
    """Handles the event to print all authorized vehicles."""
    display_vehicles(allowed_vehicles)

def handle_search_vehicle(allowed_vehicles):
    """Handles the event to search for an authorized vehicle."""
    search_vehicle(allowed_vehicles)

def handle_add_vehicle(allowed_vehicles, filename):
    """Handles the event to add a new authorized vehicle."""
    add_vehicle(allowed_vehicles, filename)

def handle_delete_vehicle(allowed_vehicles, filename):
    """Handles the event to delete an authorized vehicle."""
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            handle_print_vehicles(allowed_vehicles)
        elif choice == '2':
            handle_search_vehicle(allowed_vehicles)
        elif choice == '3':
            handle_add_vehicle(allowed_vehicles, filename)
        elif choice == '4':
            handle_delete_vehicle(allowed_vehicles, filename)
        elif choice == '5':
